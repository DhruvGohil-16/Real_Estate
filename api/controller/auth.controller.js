...
export const signupAgent = async (req, res, next) => {
  ...
  const findAgent = await agent.findOne({ "location.country": country, "location.state": state, "location.city": city });
  if (!findAgent) { 
    ...
    return res.status(201).json({success:false,message:"Agent exists"});
  }
