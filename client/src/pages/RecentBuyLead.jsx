...
export default function NewBuyLead() {
    ...
    const [recentBuyLeads,setRecentBuyLeads] = useState([]);
    ...
    const fetchRecentBuyLeads = async () => {
        setError('');
        try {
            const response = await fetch(`/api/agent/RecentBuyReq/${currentUser._id}`);
        ...
        <div className="container mx-auto px-4 py-8 mb-3 min-h-screen">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 ">
                ...
                <div className="col-span-2 md:col-span-2 bg-white shadow-lg p-6 rounded-md">
                    <h3 className="text-lg font-serif font-bold">Recent Buy Request</h3>
                    <div className='flex flex-col gap-1 mt-3 justify-center'>
                        {recentBuyLeads.length ? (
                            <div className="flex flex-col w-full h-10">
                                {recentBuyLeads.map((request,index) => (
                            ...
                            <div className="bg-slate-200 shadow-lg rounded-xl w-4/6 m-6">
                                <div className='m-6 flex flex-col'>
                                    ...
                                    <div className='flex flex-col mb-3'>
                                        <div className='flex'>
                                            <div className='flex flex-row gap-3 items-center'>
                                                <FaHome size={25} />                                                          
                                                <div className='font-serif capitalize text-start font-semibold text-black text-lg'>{recentBuyLeads[propIndex].permList.propertyType}</div>
                                            </div>
                                            <div className='font-serif capitalize text-start font-semibold text-black text-lg'> : {recentBuyLeads[propIndex].permList.propertyName}</div>
                                        </div>
                                        <div>
                                            <div className="flex flex-row gap-7 justify-start my-4">
                                                    <div className="text-gray-600 "><span className='text-black font-bold font-serif'>{Intl.NumberFormat('en-IN').format(recentBuyLeads[propIndex].permList.bedrooms)}</span> bed</div>
                                                    <div className="text-gray-600"><span className='text-black font-bold font-serif'>{Intl.NumberFormat('en-IN').format(recentBuyLeads[propIndex].permList.bathrooms)}</span> bath</div>
                                                    <div className="text-gray-600"><span className='text-black font-bold font-serif'>{Intl.NumberFormat('en-IN').format(recentBuyLeads[propIndex].permList.sqarea)}</span> sqft</div>
                                            </div>
                                            <div className='grid grid-cols-3 gap-y-2'>
                                                <div className='flex flex-row gap-3 items-center'>
                                                    <FaHome size={25} />
                                                    <div className='flex flex-col'>
                                                        <span className='font-serif font-semibold'>{recentBuyLeads[propIndex].permList.propertyType}</span>
                                                ...
                                                <div className='flex flex-row gap-3 items-center'>
                                                    ...
                                                    <div className='flex flex-col'>
                                                        <span className='font-serif font-semibold'>₹ {Intl.NumberFormat('en-IN').format((recentBuyLeads[propIndex].permList.price)/(recentBuyLeads[propIndex].permList.sqarea))}</span>
                                                        <span className='text-sm font-serif text-black/60'>Price per sqft</span>
                                                    </div>
                                                </div>
                                                {recentBuyLeads[propIndex].permList.amenities[0].parking && (
                                                        <div className='flex flex-row gap-3 items-center'>
                                                            <GiHomeGarage size={20} />
                                                            <div className='flex flex-col'>
                                                                <span className='text-lg font-serif font-semibold'>{recentBuyLeads[propIndex].permList.noOfVehicle} <span className='text-sm'>Vehicle</span></span>
                                                                <span className='text-sm font-serif text-black/60'>Parking</span>
                                                            </div>
                                                        </div>
                                                )}
                                                {recentBuyLeads[propIndex].permList.furnished && (
                                                        <div className='flex flex-row gap-3 items-center'>
                                                            <GiWoodBeam size={20} />
                                                ...
                                                <div className='flex flex-row gap-3 items-center'>
                                                    <FaHammer size={20} />
                                                    <div className='flex flex-col'>
                                                        <span className='font-serif font-semibold'>{recentBuyLeads[propIndex].permList.builtDate}</span>
                                    ...
                                    <div className='flex flex-row gap-3 items-center mb-4 pb-2'>
                                        ...
                                        <p className="text-gray-600 text-balance font-serif overflow-hidden line-clamp-3 text-ellipsis">{recentBuyLeads[propIndex].permList.address}</p>
                                    </div>
                                </div>
                                ...
                                <div className='grid grid-cols-2 mx-6'>
                                    <div className='border-r-2 border-black'>
                                        <div className='mt-2 justify-start flex flex-col'>
                                            ...
                                            <div className='flex flex-row gap-2'>
                                                <div className='text-black text-xl font-semibold font-serif'>offer : ₹{Intl.NumberFormat('en-IN').format(recentBuyLeads[propIndex].offer)}</div>
                                            </div>
                                            
                                            <div className="flex flex-col gap-3 justify-start my-4">
                                                <div className="text-gray-600 ">Name    : <span className='text-black font-bold font-serif'>{(recentBuyLeads[propIndex].buyerName)}</span></div>
                                                <div className="text-gray-600 ">Contact : <span className='text-black font-bold font-serif'>{(recentBuyLeads[propIndex].newUser.email)}</span></div>
                                    ...
                                    <div className='border-l-2 border-black'>
                                        <div className='ml-2 mt-2 justify-center flex flex-col'>
                                            ...
                                            <div className='flex flex-row gap-2'>
                                                {recentBuyLeads[propIndex].permList.offer && (
                                                    <div className='text-black text-xl font-semibold font-serif'>Ask : ₹{Intl.NumberFormat('en-IN').format(recentBuyLeads[propIndex].permList.discountPrice)}</div>
                                                )}
                                                {!recentBuyLeads[propIndex].permList.offer && (
                                                    <div className="text-black text-xl font-semibold font-serif">Ask : ₹{Intl.NumberFormat('en-IN').format(recentBuyLeads[propIndex].permList.price)}</div>
                                                )}
                                            </div>
                                            
                                            <div className="flex flex-col gap-3 justify-start my-4">
                                                <div className="text-gray-600 ">Name    : <span className='text-black font-bold font-serif'>{(recentBuyLeads[propIndex].permList.owner)}</span></div>
                                                <div className="text-gray-600 ">Contact : <span className='text-black font-bold font-serif'>{(recentBuyLeads[propIndex].permList.userEmail)}</span></div>
                                        ...
                                        <PopoverContent borderColor='black'>
                                            ...
                                            <PopoverBody>
                                                <>
                                                    ...
                                                    <button  onClick={()=>handleVerify(-1)} className='flex justify-evenly items-center text-center w-full p-2 rounded font-serif  border border-black hover:bg-gray-800 hover:text-white mb-4'>Reject</button>                                  
                                                </>
                                            </PopoverBody>
                                        </PopoverContent>
                                    </Popover>
                                    <button disabled={loading} onClick={()=>setView(false)} className="flex justify-evenly items-center w-full h-fit p-2 shadow-lg shadow-black/20 rounded font-serif  border border-black hover:bg-red-800 hover:text-white mb-4">close</button>
                                </div>
